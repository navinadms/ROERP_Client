...
Public Class InwardRegister
    ...
    Public Sub New()
        ...
        End If
        getPageRight()
        txtEntryno.Text = linq_obj.GetMaxInwardId().ToString()
    End Sub

    Public Sub AutoCompated_Text()
        txtCompanyName.AutoCompleteCustomSource.Clear()
        Dim data = linq_obj.SP_Select_All_Tbl_InwardStockMaster().ToList()
        For Each iteam As SP_Select_All_Tbl_InwardStockMasterResult In data
            txtCompanyName.AutoCompleteCustomSource.Add(iteam.SupplierName)
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        Dim mainData = linq_obj.SP_Select_All_Tbl_InwardStockMaster().Where(Function(t) t.BillDate >= dtStartDate.Text And t.BillDate <= dtEndDate.Text).ToList()
        Dim dt As New DataTable
        dt.Columns.Add("Bill No")
        dt.Columns.Add("Bill Date")
        dt.Columns.Add("Company Name")
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        Try
            If txtBillNo.Text.Trim() <> "" And InwardId = 0 Then
                ...
                If InwardId = -1 Then
                    MessageBox.Show("Bill no all ready exists")
                    Exit Sub
                End If
            End If
            If (InwardId > 0) Then
    ...
    Private Sub btnInWard_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnInWard.Click
        If InwardId > 0 Then
            ...
            cmd.CommandText = "SP_Select_Tbl_InwardStockMaster_ById"
            cmd.Connection = linq_obj.Connection
            cmd.Parameters.Add("@Pk_InwardId", SqlDbType.BigInt).Value = InwardId



            da.SelectCommand = cmd
            da.Fill(ds, "SP_Select_Tbl_InwardStockMaster_ById")

            ds.AcceptChanges()
            ds.Tables("SP_Select_Tbl_InwardStockMaster_ById").WriteXml(Application.StartupPath & "\XmlFile\SP_Select_Tbl_InwardStockMaster_ById.xml")

            cmd = New SqlCommand
            da = New SqlDataAdapter
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "SP_Select_All_Tbl_InwardDetailByInward"
            cmd.Connection = linq_obj.Connection

            cmd.Parameters.Add("@InwardId", SqlDbType.Int).Value = InwardId

            da.SelectCommand = cmd
            da.Fill(ds, "SP_Select_All_Tbl_InwardDetailByInward")
            ds.AcceptChanges()
            ds.Tables("SP_Select_All_Tbl_InwardDetailByInward").WriteXml(Application.StartupPath & "\XmlFile\SP_Select_All_Tbl_InwardDetailByInward.xml")


            ''For Check Data

            Dim rpt As New RptInWardReport
    ...
    Private Sub GDVInwardDetail_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GDVInwardDetail.CellContentClick

    End Sub

    Private Sub txtCompanyName_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCompanyName.Leave
        txtAddress.Text = ""
        If txtCompanyName.Text.Trim <> "" Then

            Dim data = linq_obj.SP_Select_All_Tbl_InwardStockMaster().ToList().Where(Function(t) t.SupplierName = txtCompanyName.Text).ToList()

            For Each iteam As SP_Select_All_Tbl_InwardStockMasterResult In data
                txtAddress.Text = iteam.SupplierAddress
